...
// NOTE:
// In pure interp, we decode instruction every time, and the point
// is that we pass the pointer to decode(), which means the last
// decode result will pollute current decode result, like rvc or
// cfc and so on. So it is important to reset the pointer in
// decode() function.
...
#define X(name, tag, a1, a2, a3, a4) [instr_##name] = &&name,
    static void *dispatch_table_pure[] = { INSTRUCTION_LIST(X) };
#undef X
